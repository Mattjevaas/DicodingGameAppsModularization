...
public struct DataExistRepository<GameLocaleDataSource: LocaleDataSource>: Repository
where
    ...
    GameLocaleDataSource.Response == [FavoriteGameModel]
{
    ...
    public func execute(request: String?) -> AnyPublisher<Bool, Error> {
        ...
        return self._localDataSource.isDataExist(type: GameRealmEntity.self, predicate: NSPredicate(format: "gameId == \(request)")).eraseToAnyPublisher()
    }
}
